...
//EKRANA BASTIRMA:
sepettekiler.forEach(({img, name, price, piece})=>{  //içerde dest yaptık(en kısa kod)
        ...
        <h5 class="card-title">${name}</h5>
             ...
             <div class="ürün-price">
                    <p class="text-warning h2">$
                      <span class="indirim-price">${(price*0.7).toFixed(2)}
                      </span>
...
function pieceButton(){
    ...
    document.querySelectorAll(".adet-controller").forEach((kutu)=>{
        ...
        plus.onclick=()=>{
            ...
            //ekranda adeti güncelledik
            adet.textContent = +adet.textContent+1


        //ürünün karttaki toplamının güncellenmesi: 
...
//CARD TOPLAM DEĞERİNİ HESAPLAMA:
...
document.querySelector(".kargo").textContent=pToplam.toFixed(2) ? shipping : 0

document.querySelector(".toplam").textContent=pToplam.toFixed(2) ? (pToplam+(pToplam*tax)+shipping) : 0
}
